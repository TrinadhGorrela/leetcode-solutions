 ...
 * Key insight: Maintain four boundaries (top, bottom, left, right); traverse the four edges of the current outer layer
 * filling values with an incrementing counter, then shrink all four boundaries inward and repeat.
 *
 * Time Complexity: O(n²) - Each of the n² cells written exactly once.
 * Space Complexity: O(n²) - The output matrix itself.
 *
 * Edge Cases Handled: n = 1 (single cell), odd n (innermost layer is a single row/col), boundary collapse after last
 * layer
 */
