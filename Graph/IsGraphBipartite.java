 ...
 * Key insight: Iterate over all nodes (handling disconnected components) and DFS-color each unvisited node with 0 or 1, forcing neighbors to the opposite color. A same-color neighbor immediately proves an odd cycle exists, so the graph is not bipartite.
 *
 ...
 *
 * Edge Cases Handled: disconnected components (each independently colored from unvisited nodes), self-loop (neighbor == current, same color detected immediately), empty graph with n = 0 (vacuously bipartite)
 */
