 ...
 * Key insight: At each recursion level, iterate over all nums and skip any already in the current partial permutation
 * (using list.contains()); when the partial list reaches length n, a complete permutation is recorded.
 *
 ...
 *
 * Edge Cases Handled: single element (returns [[element]]), all distinct values (n! results), n = 0 (returns empty list
 * via size check), elements at every position in the partial list are checked for reuse
 */
class Permutations {
